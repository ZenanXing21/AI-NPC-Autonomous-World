...
class ChaseState:
	...
	func physics_update(npc, delta: float) -> void:
		if npc.can_sense_player():
			...
			npc.move_along_path(delta)
			if npc.global_position.distance_to(npc.player.global_position) <= npc.chase_distance:
				print("[FSM] Close to player while chasing")
		else:
			npc.lost_player_timer += delta
			if npc.lost_player_timer >= npc.lose_player_after_seconds:
				print("[FSM] Player lost. Returning to patrol")
				npc.change_state("PatrolState")
				return
			npc.move_along_path(delta)
...
func _ready() -> void:
	...
	if player == null:
		push_warning("[FSM] No player node assigned/found; NPC will stay in patrol behavior.")
